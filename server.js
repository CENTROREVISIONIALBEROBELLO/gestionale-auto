...
app.use(session({ secret: 'gestionale-auto-2024', resave: false, saveUninitialized: false, cookie: { maxAge: 8 * 60 * 60 * 1000 } }))

const soloAdmin   = (req, res, next) => req.session.ruolo === 'admin' ? next() : res.status(401).json({ errore: 'Non autorizzato' })
const soloLoggato = (req, res, next) => req.session.userId ? next() : res.status(401).json({ errore: 'Non autorizzato' })
...
app.get('/api/clienti', soloAdmin, (req, res) => res.json(leggiDB().clienti))
app.post('/api/clienti', soloAdmin, (req, res) => {
  const db = leggiDB()
  const { nome, email, telefono, username, password } = req.body
  if (!nome || !username || !password) return res.status(400).json({ errore: 'Campi obbligatori mancanti' })
...
app.get('/api/pratiche', soloLoggato, (req, res) => {
  const db = leggiDB()
  if (req.session.ruolo === 'admin') return res.json(db.pratiche)
...
app.get('/api/pratiche/:id', soloLoggato, (req, res) => {
  ...
  if (req.session.ruolo !== 'admin' && p.clienteId !== req.session.userId) return res.status(403).json({ errore: 'Non autorizzato' })
  res.json(req.session.ruolo === 'cliente' ? { ...p, files: (p.files||[]).filter(f => f.visibile) } : p)
})
app.post('/api/pratiche', soloAdmin, (req, res) => {
  const db = leggiDB(); const { titolo, clienteId, tipo, targa, stato, scadenza, note, numero } = req.body
...
app.post('/api/pratiche/:id/files', soloAdmin, upload.array('files', 20), (req, res) => {
  const db = leggiDB(); const p = db.pratiche.find(x => x.id === req.params.id)
  if (!p) return res.status(404).json({ errore: 'Pratica non trovata' })
  const nuovi = req.files.map(f => ({ id: Date.now() + '-' + Math.random().toString(36).slice(2), nome: f.originalname, url: '/uploads/' + f.filename, dim: f.size, visibile: false }))
...
app.post('/api/pratiche/:id/messaggi', soloLoggato, (req, res) => {
  ...
  const msg = { id: Date.now(), da: req.session.ruolo, testo: req.body.testo, ts }
